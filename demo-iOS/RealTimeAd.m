...
#define requsetUrl      @"http://api.cloudmobi.net:30001/api/v1/realtime/m/get"

@class AdResponseADListModel;
@implementation RealTimeAd
...
- (void)getAdModelWithSlotID:(NSString *)slotid adNumber:(NSInteger)adNumber imgW:(NSInteger)imageW imgH:(NSInteger)imageH successResponse:(void(^)(NSArray *adArray))successResponse failer:(void(^)(NSError *error))failer{
    AdRequsetModel *req = [[AdRequsetModel alloc]init];
    NSMutableDictionary* dic = [[NSMutableDictionary alloc]init];
    [dic setObject:[NSString stringWithFormat:@"%ld",adNumber] forKey:@"adnum"];
    [dic setObject:[NSString stringWithFormat:@"%ld",req.nt] forKey:@"nt"];
    [dic setObject:req.icc forKey:@"icc"];
    [dic setObject:req.idfa forKey:@"idfa"];
    [dic setObject:slotid forKey:@"token"];
    [dic setObject:@"iOS" forKey:@"os"];
    [dic setObject:[NSString stringWithFormat:@"%lf",req.osv] forKey:@"osv"];
    [dic setObject:@"phone" forKey:@"dt"];
    [dic setObject:[NSString stringWithFormat:@"%ld",req.nt] forKey:@"nt"];
    [dic setObject:[NSString stringWithFormat:@"%ld",imageW] forKey:@"imgw"];
    [dic setObject:[NSString stringWithFormat:@"%ld",imageH] forKey:@"imgh"];
    [dic setObject:req.pn forKey:@"pn"];
    [dic setObject:@"1.0" forKey:@"sv"];
    [dic setObject:@"keywords" forKey:@"keywords"];
    [self GET:requsetUrl parameters:dic completeHandler:^(id responseObj, NSError *error) {
        if (error) {
            failer(error);
        }else{
            if (responseObj != nil && ![responseObj isKindOfClass:[NSNull class]]) {
                if ([[responseObj valueForKey:@"err_no"] integerValue] == 0) {
                    NSArray *adArr = [responseObj valueForKey:@"ad_list"];
                    NSMutableArray *arr = [[NSMutableArray alloc]init];
                    for (int i = 0; i <adArr.count; i ++) {
                        AdResponseADListModel *adlistModel = [[responseObj valueForKey:@"ad_list"] objectAtIndex:i];
                        AdResponseADListModel *ad = [[AdResponseADListModel alloc]init];
                        ad.icon = [adlistModel valueForKey:@"icon"];
                        ad.landing_type = [[adlistModel valueForKey:@"landing_type"] integerValue];
                        ad.clk_url = [adlistModel valueForKey:@"clk_url"];
                        ad.imp_tks = [adlistModel valueForKey:@"imp_tks"];
                        ad.clk_tks = [adlistModel valueForKey:@"clk_tks"];
                        ad.icon_size = [adlistModel valueForKey:@"icon_size"];
                        ad.title = [adlistModel valueForKey:@"title"];
                        ad.pkg_name = [adlistModel valueForKey:@"pkg_name"];
                        ad.image = [adlistModel valueForKey:@"image"];
                        ad.image_size = [adlistModel valueForKey:@"image_size"];
                        ad.desc = [adlistModel valueForKey:@"desc"];
                        ad.button = [adlistModel valueForKey:@"button"];
                        [arr addObject:ad];
                    }
                    if (arr.count == 0) {
                        arr = nil;
                    ...
                    NSError *infoerr = [[NSError alloc]initWithDomain:[responseObj valueForKey:@"err_msg"] code:[[responseObj valueForKey:@"err_no"] integerValue] userInfo:nil];
                    failer(infoerr);
                }
            }
        }
...
//Get request Method
- (id)GET:(NSString *)path parameters:(NSDictionary *)params completeHandler:(void(^)(id responseObj, NSError* error))complete{
    ...
    requestPath = (NSMutableString*)[requestPath substringWithRange:NSMakeRange(0,requestPath.length-1)];
    requestPath = [ NSMutableString stringWithString:[requestPath stringByReplacingOccurrencesOfString:@" " withString:@""]];
    NSURL* url = [NSURL URLWithString:requestPath];
    return [self GET:url completeHandler:^(id responseObj, NSError *error) {
        if (error) {
            complete(nil,error);
        }
        else
...
//NSURLSession
- (NSURLSessionDataTask*)GET:(NSURL *)url completeHandler:(void(^)(id responseObj, NSError *error))complete;
{
...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        //如果关闭广告限制则调用openidfa方法获取一个
        idfa = (osv == 10.0)&& ![[ASIdentifierManager sharedManager] isAdvertisingTrackingEnabled] ? [OpenIDFA sameDayOpenIDFA] : [[[ASIdentifierManager sharedManager] advertisingIdentifier] UUIDString];
        dt = [[UIDevice currentDevice] model];
        icc =  [[NSLocale currentLocale] objectForKey:NSLocaleCountryCode];
        nt = [self getNetWorkState];
        pn = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleIdentifier"];
    }
    return self;
}
//Get netWork state
- (int)getNetWorkState
{
    ...
    if (nil !=subviews && ![subviews isKindOfClass:[NSNull class]])
    {
...
//AD Response Model
@implementation AdResponseModel
@end

@implementation AdResponseADListModel
@end
